...
# The idea is to exploration vs exploitation tradeoff. We need to explore the arms
# that we have not explored enough and exploit the arms that we know are good.

# the idea is to get a net positive reward.
# for initial number of steps we can explore all the arms for epsilon number of times
# if we give a single arm the the net reward is -(1-pr)
# 
...
class CostlySetBanditsAlgo(Algorithm):
    ...
    def give_query_set(self):
        ...
        for i in range(self.num_arms):
            # penalty_value = 1/(i + 1)
            # this is the minimum value of a sorted set 
            # sorted_sub_values.append(beta_values[sorted_arms[i]] - penalty_value)
