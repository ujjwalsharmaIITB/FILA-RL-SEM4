...
def feature_extraction(state, info):
    ...
    # dynamic_look_ahead_rows = min(12, int(speed // 4) + 8)

    # print("dynamic_look_ahead_rows", dynamic_look_ahead_rows)
    ...
    avg_center_point_for_last_rows = np.mean(rowCentered[9:12])
    
    # 8 = avg_center_point_for_top_rows / 8
    avg_center_point_for_top_rows = np.mean(rowCentered[4:9])
    ...
    steering_intensity =  np.mean(rowCenteresWeighted[9:12]) # Steering intensity based on the distance from the center of the lane
    
    curvature = np.mean(rowCentered[9:]) - np.mean(rowCentered[5:10]) 

    speed = info["speed"]  # Current speed of the car

    # denominator_for_acceleration_intensity = (abs(mean_point - avg_center_point_for_last_rows) + 1)
    denominator_for_acceleration_intensity = (abs(curvature) + 1)
    ...
    return rowCenteresWeighted, rowCentered, avg_center_point, avg_center_point_for_top_rows, avg_center_point_for_last_rows, steering_intensity, speed, acceleration_intensity, curvature
...
def policy(state, info, eval_mode = False, params = []):
    ...
    if eval_mode:
        param_df = pd.read_json("cmaes_params.custom.json")
    ...
    rowCenteresWeighted, rowCentered, avg_center_point, avg_center_point_for_top_rows, avg_center_point_for_last_rows, steering_intensity, speed, acceleration_intensity, curvature = feature_extraction(state, info)

    steering_features = np.array(rowCenteresWeighted + [curvature, steering_intensity, avg_center_point_for_last_rows])  # Features for steering 28

    steering_feature_count = len(steering_features)
    steering_unsqueezed = np.dot(steering_features, params[:steering_feature_count])  # Steering is a linear combination of features
    ...
    acceleration_features = np.array(rowCenteresWeighted + [speed, curvature, avg_center_point_for_top_rows, avg_center_point_for_last_rows, acceleration_intensity]) # 

    acceleration_unsqueezed = np.dot(acceleration_features, params[steering_feature_count:])  # Acceleration is a linear combination of features
...
if __name__ == '__main__':
    ...
    if train_mode:
        ...
        cmaes_params_df.to_json("cmaes_params.custom.json")  # Storing parameters for evaluation purpose
    ...
    if rendering:
        env = RecordVideo(env, video_folder="videos.13april", name_prefix="eval", episode_trigger=lambda x: True)
